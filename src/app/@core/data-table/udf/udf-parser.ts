...
function intersection(arrays: any[]): any[] {
  return arrays.reduce(function (a, b) {
    return a.filter(function (value: any) {
...
function parser2(v: any[], inverse: boolean = false): any[] {
  return v.map(ele => {
    if (Array.isArray(ele)) {
      return ele[0] === '!'
        ? parser2(ele, inverse).filter(Boolean)
        : parser2(ele, inverse);
    }
    if (isComparisonOperator(ele)) {
      return getComparisonOperator(ele, inverse);
    }
    if (isLogicalOperator(ele)) {
      return getLogicalOperator(ele, inverse);
    }
    if (ele === '!') {
      inverse = true;
      return null;
...
function parser3(v: any[]): any[] {
  if (Array.isArray(v) && isComparisonOperator(v[1])) {
    return [buildPredicate(v[0], v[1], v[2])];
  }
  return v.map(ele => {
    if (Array.isArray(ele)) {
 ...
 *
 * @param v description
 * @param udfFilterable description
 */
function parser4(
  v: any[],
  udfFilterable: UdfFilterable = new UdfConjunction()
): UdfFilterable {
  udfFilterable = hasAndOperator(v)
    ? new UdfDisjunction()
    : new UdfConjunction();
  v.forEach(ele => {
    if (Array.isArray(ele)) {
...
function getComparisonOperator(op: string, inverse: boolean = false): string {
  return inverse
    ? comparisonOperators[1][comparisonOperators[0].indexOf(op)]
    : op;
}
